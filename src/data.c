...
box_label *read_boxes(char *filename, int *n)
{
    ...
    int id;
    int count = 0;
    while (fscanf(file, "%d %f %f %f %f %f", &id, &x, &y, &w, &h, &s) == 6)
    {
                          ...
                          int net_w, int net_h)
{
    ...
    for (i = 0; i < count; ++i)
    {
        ...
        if (s < 0 || s > 80)
        {
            printf("\n Wrong annotation: x = %f \n", s);
            sprintf(buff, "echo %s \"Wrong annotation: distance = %f\" >> bad_label.list", labelpath, s);
            system(buff);
            ++sub;
            if (check_mistakes)
                getchar();
