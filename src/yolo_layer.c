...
void delta_yolo_class(float *output, float *delta, int index, int class_id, int classes, int stride, float *avg_cat, int focal_loss)
{
    ...
    {
        delta[index + stride * class_id] = (1 - output[index + stride * class_id]) * 1.5;
        if (avg_cat)
    ...
    else
    {
        ...
        for (n = 0; n < classes; ++n)
        {
            delta[index + stride * n] = (((n == class_id) ? 1 : 0) - output[index + stride * n]) * 1.5;
...
void delta_yolo_distance(float *output, float *delta, int index, float distance, int stride, float *avg_cat, int focal_loss)
{
    // float distance_thresh = distance * 0.05;
    // printf("original distance %f \n", distance);
    // printf("estimated distance %f \n", output[index + stride * 0]);
    const float scale = 12.0;
    if (delta[index + stride * 0])
    {
        delta[index + stride * 0] = (distance - output[index + stride * 0]) * scale;
        if (avg_cat)
            *avg_cat += output[index + stride * 0];
        return;
    }
    // Focal loss
    if (focal_loss)
    {
        ...
        int ti = index + stride * 0;
        float pt = output[ti] + 0.000000000000001F;
        // http://fooplot.com/#W3sidHlwZSI6MCwiZXEiOiItKDEteCkqKDIqeCpsb2coeCkreC0xKSIsImNvbG9yIjoiIzAwMDAwMCJ9LHsidHlwZSI6MTAwMH1d
        float grad = -(1 - pt) * (2 * pt * logf(pt) + pt - 1); // http://blog.csdn.net/linmingan/article/details/77885832
        //float grad = (1 - pt) * (2 * pt*logf(pt) + pt - 1);    // https://github.com/unsky/focal-loss

        // delta[index + stride * 0] = (((pt >= distance-distance_thresh && pt <= distance + distance_thresh) ? 1 : 0) - output[index + stride * 0]);
        delta[index + stride * 0] = (distance - output[index + stride * 0]) * scale;

        delta[index + stride * 0] *= alpha * grad;
    ...
    for (b = 0; b < l.batch; ++b)
    {
            ...
            for (i = 0; i < l.w; ++i)
            {
                for (n = 0; n < l.n; ++n)
                {
                    ...
                    for (t = 0; t < l.max_boxes; ++t)
                    {
                        ...
                        float distance = state.truth[t * (4 + 1 + 1) + b * l.truths + 5];
                    ...
                    if (best_iou > l.truth_thresh)
                    {
                        ...
                        float distance = state.truth[best_t * (4 + 1 + 1) + b * l.truths + 5];
                        if (l.map)
                            class_id = l.map[class_id];
                        int class_index = entry_index(l, b, n * l.w * l.h + j * l.w + i, 4 + 1);
                        int distance_index = entry_index(l, b, n * l.w * l.h + j * l.w + i, 4 + 1 + 1);
                        delta_yolo_class(l.output, l.delta, class_index, class_id, l.classes, l.w * l.h, 0, l.focal_loss);
                        delta_yolo_distance(l.output, l.delta, distance_index, distance, l.w * l.h, 0, l.focal_loss);
        ...
        for (t = 0; t < l.max_boxes; ++t)
        {
            ...
            if (mask_n >= 0)
            {
                ...
                int class_id = state.truth[t * (4 + 1 + 1) + b * l.truths + 4];
                float distance = state.truth[t * (4 + 1 + 1) + b * l.truths + 4 + 1];
                if (l.map)
                    class_id = l.map[class_id];
                int class_index = entry_index(l, b, mask_n * l.w * l.h + j * l.w + i, 4 + 1);
                int distance_index = entry_index(l, b, mask_n * l.w * l.h + j * l.w + i, 4 + 1 + 1);
                delta_yolo_class(l.output, l.delta, class_index, class_id, l.classes, l.w * l.h, 0, l.focal_loss);
                delta_yolo_distance(l.output, l.delta, distance_index, distance, l.w * l.h, 0, l.focal_loss);
    ...
    for (i = 0; i < l.w * l.h; ++i)
    {
        ...
        for (n = 0; n < l.n; ++n)
        {
            ...
            if (objectness > thresh)
            {
                ...
                int dist_index = entry_index(l, 0, n * l.w * l.h + i, 4 + 1 + 1);
                dets[count].bbox = get_yolo_box(predictions, l.biases, l.mask[n], box_index, col, row, l.w, l.h, netw, neth, l.w * l.h);
                dets[count].objectness = objectness;
                dets[count].classes = l.classes;
                dets[count].distance = predictions[dist_index] * 30.0;
